...
states that electrons trapped in a parabolic quantum well behaves as if they were 
...
oscillator $\omega=\Omega=1$, to ensure a population of excited states. In this simulation 
...
\begin{figure}
    ...
    \caption{Fourier transform of expected value of dipole moment for 
        ...
        are computed relavtive to the largest system ($n=12$).
    }
...
of the system was allowed to continue to $T=30 \text{ au}$. The maximum intensity 
of the oscillating field was lowered to $E_\text{max} = 0.25 \text{ a.u.}$,
to ensure stabiliy of the simulations. 
...
to $\Omega=1$ and the frequency of the electric field to $\omega=2\Omega=2 \text{ a.u.}$.
We 
...
\begin{figure}
    ...
    \caption{Ground state probability $|\braket{\Psi(0)}{\Psi(t)}|^2$ for a two-dimensional
...
The reason for the problems the TDCCSD methods shows are likely caused by the 
...
The ground state probability of the same simulation with the 
...
\begin{figure}
    ...
    \caption{Norm of amplitudes, $\tau_1$, $\tau_2$, $\lambda_1$ and
        $\lambda_2$ computed with the TDCCSD method in an $n=12$ electron
        two-dimensional quantum 
        dot influenced by an oscilalting field with frequency $\omega = 2 \text{ a.u.}$,
...
in probability values that are unreasonable. In this case we do believe the 
energy plots to be correct for $\omega\in\{1.5, 1.75,2.0\}$. 
We have less trust in the results from the last two simulations where the oscillating 
field had frequencies $\omega\in\{1.0, 1.25\}$. Here we see the same illogical time-dependent 
...
see that the larger system has a relavtively higher dipole frequency 
...
of a magnetic field. Except for the magnetic field the systems are idendical with the 
...
in atomic units.
For each of these simulations we compute the Fourier transform of the expectation 
