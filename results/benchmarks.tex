...
Coulomb interaction. We pick a wide grid of $300 \text{ au}$, with $5001$ points.

The idea behind the simulation is to comptute the particle density over time, and see if 
...
$\rho(x, t)$ is computated at the very beginning of the simulation, halfway through 
...
from the reference state $\ket{\Phi_0}$ which should provide the best representation of the 
exact state $\ket{\Psi}$. For this reason one would not expect the norm of the 
amplitudes to be greater than $1$. In \autoref{fig:miyagi_tddccsd_amplitude} we see
that the amplitudes stay within a reasonable magnitude for up to about halfway through 
the simulation, after which the which we see the method struggling greatly to 
...
\citeauthor{pedersen2019symplectic}\cite{pedersen2019symplectic} draw a similar 
conlusion, highlighting there appears to be system-dependent upper limit for 
the strenght of the external field. They underline the improvement in the computations 
