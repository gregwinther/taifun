...
set\footnote{In quantum chemistry, there exists several specialised basis sets. For a
...
($E_\text{max} = 0.07 \text{ a.u.}$). \citeauthor{li2005time} also run a simulation 
...
which it appear to be. For comparison we have computed the result with both of 
...
maximum intensity of $\vb{E}_\text{max} = 1$ and a laser frequency of
$\omega = 8 \Omega = 2$.
...
\begin{figure}
    ...
    \caption{
        ...
        Electron density in dimensionless units for the ground state wavefunction of
        a quantum dot with 
...
\begin{figure}
    ...
    \caption{
        ...
        This plot corresponds precisely with 
...
time is dervied from the Hartree unit for energy $E_a$, which we derived in
...
orbitals in time enable us to get the same qualitative result as \citeauthor{miyagi2013time}
...
We use the exact same integrator as the one Pedersen and Kvaal outline.
