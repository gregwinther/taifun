...
See Kvaal\cite{kvaal2013variational}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
In contrast with the expansions if the coupled cluster amplitude equations, which 
...
Now we wish to show that Arponen's framework corresponds to that of Helgakger and 
...
\begin{equation}
    ...
    \mathscr{E}_{\hat{H}}(\tau^\dagger, \tau, \tilde{\Phi}, \Phi)
    = \frac{\mel{\tilde{\Phi}}{e^{\hat{T}\dagger}\hat{H}e^{\hat{H}}}{\Phi}}
    {\mel{\tilde{\Phi}}{e^{\hat{T}^\dagger}e^{\hat{T}}}{\Phi}}.
\end{equation}
...
\begin{equation}
    ...
    \quad
    \tilde{c}^\dagger_p = \int \tilde{\phi}_p(\vb{x}) \Psi(\vb{x})d\vb{x},
\end{equation}
where $\Psi^\dagger$ and $\Psi$ are field creation- and annihilation operators.
These second quantised operators are subject to the anticommutator relation,
\begin{equation}
    \{\tilde{c}_p, c_q^\dagger \} \equiv \tilde{c}_p c_q^\dagger + c_q^\dagger\tilde{c}_p
    \stackrel{!}{\equiv} \braket*{\tilde{\phi}_p}{\phi_q} = \delta_{pq} = \delta_{pq}.
\end{equation}

NEXT: The special projection operator and how the cluster operator depend on 
the orbitals..! 